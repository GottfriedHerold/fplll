...
public:
  ...
  friend std::ostream &operator<< <SieveTraits, false>(std::ostream &, GaussQueue<SieveTraits,false> const &);
...
private:
  ...
  // mutable, because stream output needs to modify and restore the queue.
  mutable QueueType main_queue;                  // actual queue of lattice points to be processed.
